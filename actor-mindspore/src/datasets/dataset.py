...
from ..utils.misc import to_ms
import src.utils.rotation_conversions as geometry

POSE_REPS = ["xyz", "rotvec", "rotmat", "rotquat", "rot6d"]

# mindspore 使用迭代对象进行数据的读取, 无需像 torch 一样继承 Dataset
# 需要实现迭代对象的 __getitem__ 方法(注意: mindspore 的 __getitem__ 方法只支持返回 tuple(numpy.ndarray))
class Dataset:
    ...
    def get_pose_data(self, data_index, frame_ix):
        ...
        return pose, label  # pose: tensor, label: int

    def get_label(self, ind):
        action = self.get_action(ind)
        return self.action_to_label(action)  # 返回 int
    ...
    def __getitem__(self, index):
        ...
        inp, target = self._get_item_data_index(data_index)  # inp: tensor, target: int
        inp, target = inp.numpy(), np.array(target)
        return tuple([inp, target])
    ...
    def _load(self, ind, frame_ix):
        ...
        if pose_rep == "xyz" or self.translation:
            if getattr(self, "_load_joints3D", None) is not None:
                ...
                ret = to_ms(joints3D)  # 将 joint3D 转换为 ms.Tensor
                if self.translation:
                    ret_tr = ret[:, 0, :]
            else:
                if pose_rep == "xyz":
        ...
        if pose_rep != "xyz" and self.translation:
            padded_tr = ms.ops.zeros((ret.shape[0], ret.shape[2]), ret.dtype)
            padded_tr[:, :3] = ret_tr
            ret = ms.ops.concat((ret, padded_tr[:, None]), 1)
        ret = ret.permute(1, 2, 0)
        return ret.float()  # 返回 tensor
    ...
    def _get_item_data_index(self, data_index):
        ...
        return inp, target  # inp: tensor, target: int
