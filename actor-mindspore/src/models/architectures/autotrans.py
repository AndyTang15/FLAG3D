...
def subsequent_mask(size: int):
    ...
    return ms.Tensor.from_numpy(mask) == 0
...
def augment_x(x, y, mask, lengths, num_classes, concatenate_time):
    ...
    if len(y.shape) == 1:  # can give on hot encoded as input
        ...
        # one_hot = ops.OneHot()
        y = ops.one_hot(y, num_classes, on_value=ms.Tensor(1, ms.float32), off_value=ms.Tensor(0, ms.float32))
    y = y.to(dtype=x.dtype)
    y = y[:, None, :].repeat(nframes, axis=1)

    if concatenate_time:
        # Time embedding
        time = mask * 1 / (lengths[..., None] - 1)
        # time = (time[:, None] * torch.arange(time.shape[1], device=x.device)[None, :])[:, 0]
        time = (time[:, None] * ops.arange(time.shape[1])[None, :])[:, 0]
        time = time[..., None]
        # x_augmented = torch.cat((x, y, time), 2)
        x_augmented = ops.concat((x, y, time), 2)
    else:
        # x_augmented = torch.cat((x, y), 2)
        x_augmented = ops.concat((x, y), 2)
    return x_augmented
...
def augment_z(z, y, mask, lengths, num_classes, concatenate_time):
    ...
    z_augmented = ops.concat((z, y), 1)[:, None].repeat(mask.shape[1], 1)
    ...
    if concatenate_time:
        ...
        z_augmented = ops.concat((z_augmented, time[..., None]), 2)

    return z_augmented


class Decoder_AUTOTRANS(nn.Cell):
    def __init__(self, modeltype, njoints, nfeats, num_frames, num_classes, translation, pose_rep, glob, glob_rot,
                 concatenate_time=True, positional_encoding=True, latent_dim=256, ff_size=1024, num_layers=4,
                 num_heads=4,
        ...
        self.embedding = nn.Dense(self.input_feats, self.latent_dim)
        # self.embedding_x = nn.Linear(self.input_feats_x, self.latent_dim)
        self.embedding_x = nn.Dense(self.input_feats_x, self.latent_dim)
        ...
        self.layers = nn.CellList([TransformerDecoderLayer(size=self.latent_dim,
                                                           ff_size=self.ff_size,
                                                           num_heads=self.num_heads,
                                                           dropout=self.dropout)
                                   for _ in range(self.num_layers)])
        ...
        self.layer_norm = nn.LayerNorm(self.latent_dim, epsilon=1e-6)

        self.emb_dropout = nn.Dropout(keep_prob=self.emb_dropout)
        # self.output_layer = nn.Linear(self.latent_dim, self.output_feats, bias=False)
        self.output_layer = nn.Dense(self.latent_dim, self.output_feats, has_bias=False)
    ...
    def construct(self, batch):
        ...
        # in eval mode, by default it is not unless it is "forced"
        ...
        if teacher_forcing:
            ...
            x = ops.concat((x.new_zeros((x.shape[0], 1, *x.shape[2:])), x[:, :-1]), axis=1)
            # Embedding of the input
            x_augmented = augment_x(x, y, mask, lengths, self.num_classes, self.concatenate_time)
            trg = self.embedding_x(x_augmented)
            trg_mask = (mask[:, None] * subsequent_mask(lenseqmax).astype(mask.dtype))
            ...
            x = ms.Tensor.new_zeros(z, (bs, 1, njoints, nfeats))
            for index in range(lenseqmax):
                ...
                x = ops.concat((x, last_out[:, None]), 1)
