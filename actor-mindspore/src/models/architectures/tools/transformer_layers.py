...
class MultiHeadedAttention(nn.Cell):
    ...
    def __init__(self, num_heads: int, size: int, dropout: float = 0.1):
        ...
        self.k_layer = nn.Dense(size, num_heads * head_size)
        self.v_layer = nn.Dense(size, num_heads * head_size)
        self.q_layer = nn.Dense(size, num_heads * head_size)

        self.output_layer = nn.Dense(size, size)
        self.softmax = nn.Softmax(axis=-1)
        self.dropout = nn.Dropout(dropout)

    def construct(self, k: Tensor, v: Tensor, q: Tensor, mask: Tensor = None):
        """
        ...
        k = k.view(batch_size, -1, num_heads, self.head_size).transpose(0, 2, 1, 3)
        v = v.view(batch_size, -1, num_heads, self.head_size).transpose(0, 2, 1, 3)
        q = q.view(batch_size, -1, num_heads, self.head_size).transpose(0, 2, 1, 3)
        ...
        # back to [B, M, D]
        context = ops.matmul(attention, v)  # torch.Size([48, 8, 183, 32])
...
class PositionwiseFeedForward(nn.Cell):
    ...
    def __init__(self, input_size, ff_size, dropout=0.1):
        ...
        self.pwff_layer = nn.SequentialCell(
            nn.Dense(input_size, ff_size),
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Dense(ff_size, input_size),
            nn.Dropout(dropout),
        )
...
class PositionalEncoding(nn.Cell):
    ...
    def __init__(self,
        ...
        # zeros = ops.Zeros()
        pe = ops.zeros((max_len, size), ms.float32)
        position = ms.numpy.arange(0, max_len).unsqueeze(1)
        div_term = ops.exp((ms.numpy.arange(0, size, 2, dtype=ms.float32) *
...
class TransformerDecoderLayer(nn.Cell):
    ...
    def __init__(self,
        ...
        self.x_layer_norm = nn.LayerNorm(size, epsilon=1e-6)
        self.dec_layer_norm = nn.LayerNorm(size, epsilon=1e-6)
