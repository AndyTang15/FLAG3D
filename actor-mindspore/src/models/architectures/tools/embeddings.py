...
class Softplus(ms.nn.Cell):
    def __init__(self, beta=1, threhold=20):
    ...
    def construct(self, x):
        x = x.numpy()
        # x[x * self.beta > self.threhold] = x
        x[x * self.beta <= self.threhold] = 1 / self.beta * ops.log(
            1 + ops.exp(self.beta * x[x * self.beta <= self.threhold]))
        return ms.Tensor.from_numpy(x)
...
class MaskedNorm(nn.Cell):
    ...
    def construct(self, x: Tensor, mask: Tensor):
        if self.training:
            ...
            selected = ops.masked_select(reshaped, reshaped_mask).reshape(
                [-1, self.num_features]
            )
            batch_normed = self.norm(selected)
            # scattered = reshaped.masked_scatter(reshaped_mask, batch_normed)
            scattered = reshaped.copy().numpy()
            scattered[reshaped_mask.numpy().flatten()] = batch_normed.numpy()
            scattered = ms.Tensor.from_numpy(scattered)
...
class Embeddings(nn.Cell):
    ...
    def construct(self, x: Tensor, mask: Tensor = None) -> Tensor:
        """
...
class SpatialEmbeddings(nn.Cell):
    """
    ...
    def __init__(
        ...
        self.ln = nn.Dense(self.input_size, self.embedding_dim)
