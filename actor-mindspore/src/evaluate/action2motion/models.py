...
def test():
    ...
    for i in range(1000):
        ...
        # classifier 是 torch Netwok
        data = torch.from_numpy(data.numpy())
        lengths = torch.from_numpy(lengths.numpy())
        xyz_t = torch.from_numpy(xyz_t.numpy())
        predicted_cls_xyz = classifier(data, lengths=lengths).argmax().item()  # item() 表示返回基本数值(int/float)
    ...
    accuracy_xyz = (ms.Tensor.trace(confusion_xyz)/ops.reduce_sum(confusion_xyz)).numpy().item()
    accuracy = (ms.Tensor.trace(confusion)/ops.reduce_sum(confusion)).numpy().item()
