...
class Encoder_FC(nn.Cell):
    def __init__(self, modeltype, njoints, nfeats, num_frames, num_classes, translation, pose_rep, glob, glob_rot,
        ...
        #                                      nn.Linear(512, 256),
        #                                      nn.GELU())
        self.fully_connected = nn.SequentialCell(nn.Dense(self.input_dim, 512),
                                                 nn.GELU(),
                                                 nn.Dense(512, 256),
                                                 nn.GELU())
        if self.modeltype == "cvae":
            ...
            self.mu = nn.Dense(256, self.latent_dim)
            self.var = nn.Dense(256, self.latent_dim)
        else:
            # self.final = nn.Linear(256, self.latent_dim)
            self.final = nn.Dense(256, self.latent_dim)
...
class Decoder_FC(nn.Cell):
    def __init__(self, modeltype, njoints, nfeats, num_frames, num_classes, translation, pose_rep, glob, glob_rot,
        ...
        #                                      nn.GELU())
        self.fully_connected = nn.SequentialCell(nn.Dense(self.input_dim, 256),
                                                 nn.GELU(),
                                                 nn.Dense(256, 512),
                                                 nn.GELU(),
                                                 nn.Dense(512, self.output_dim),
                                                 nn.GELU())
