...
def augment_x(x, y, mask, lengths, num_classes, concatenate_time):
    ...
    if len(y.shape) == 1:  # can give on hot encoded as input
        ...
        y = ops.one_hot(y, num_classes, on_value=ms.Tensor(1, dtype=ms.float32), off_value=ms.Tensor(0, dtype=ms.float32))
    y = y.to(dtype=x.dtype)
    y = y[:, None, :]
    y = ms.numpy.tile(y, (1, nframes, 1))

    if concatenate_time:
        # Time embedding
        time = mask * 1 / (lengths[..., None] - 1)
        # time = (time[:, None] * torch.arange(time.shape[1], device=x.device)[None, :])[:, 0]
        time = (time[:, None] * ms.ops.arange(time.shape[1])[None, :])[:, 0]
        time = time[..., None]
        # x_augmented = torch.cat((x, y, time), 2)
        x_augmented = ms.ops.concat((x, y, time), 2)
    else:
        # x_augmented = torch.cat((x, y), 2)
        x_augmented = ms.ops.concat((x, y), 2)
    return x_augmented
...
def augment_z(z, y, mask, lengths, num_classes, concatenate_time):
    ...
    z_augmented = ms.ops.concat((z, y), 1)[:, None]
    z_augmented = ms.numpy.tile(z_augmented, (1, mask.shape[1], 1))
    ...
    if concatenate_time:
        ...
        z_augmented = ms.ops.concat((z_augmented, time[..., None]), 2)

    return z_augmented


class Encoder_GRU(nn.Cell):
    def __init__(self, modeltype, njoints, nfeats, num_frames,
        ...
        self.feats_embedding = nn.Dense(self.input_feats, self.latent_dim)
        ...
        if self.modeltype == "cvae":
            ...
            self.mu = nn.Dense(self.latent_dim, self.latent_dim)
            self.var = nn.Dense(self.latent_dim, self.latent_dim)
        else:
            self.final = nn.Dense(self.latent_dim, self.latent_dim)
    ...
    def construct(self, batch):
        ...
        x = x[tuple(ops.stack((ms.ops.arange(bs), lengths - 1)))]
...
class Decoder_GRU(nn.Cell):
    def __init__(self, modeltype, njoints, nfeats, num_frames,
        ...
        self.final_layer = nn.Dense(self.latent_dim, self.output_feats)
