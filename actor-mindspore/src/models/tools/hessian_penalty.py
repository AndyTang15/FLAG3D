...
@FileName: hessian_penalty.py.py
...
# 注意: torch.max 和 ma.ops.max 的输出相反:
#      torch.max 输出的第一项是 数值; 第二项是 index; 而 ms.ops.max 输出的第一项是 index, 第二项是 数值
def hessian_penalty(G, batch, k=2, epsilon=0.1, reduction=ms.ops.max, return_separately=False, G_z=None, **G_kwargs):
    """
    Official Mindspore Hessian Penalty implementation.
    ...
    z = batch["x"]
    # TODO: 理解代码 torch.Size
    # rademacher_size = torch.Size((k, *z.size()))  # (k, N, z.size())
    rademacher_size = (k, *z.size())
    dzs = epsilon * rademacher(rademacher_size)
    second_orders = []
...
def rademacher(shape):
    ...
    # x = ms.numpy.empty(shape)
    # x.random_(0, 2)  # Creates random tensor of 0s and 1s
    x = np.random.randint(0, 2, shape)
...
def stack_var_and_reduce(list_of_activations, reduction=ms.ops.max):
    """Equation (5) from the paper."""
    second_orders = ms.ops.stack(list_of_activations)  # (k, N, C, H, W)
    # var_tensor = torch.var(second_orders, dim=0, unbiased=True)  # (N, C, H, W)
    var_tensor = ms.Tensor.var(second_orders, axis=0)  # (N, C, H, W)

    # penalty = reduction(var_tensor)  # (1,) (scalar)
    penalty = var_tensor.max()
    return penalty


def multi_stack_var_and_reduce(sdds, reduction=ms.ops.max, return_separately=False):
...
def _test_hessian_penalty():
    ...
    batch_size = 10
    nz = 2
    z = ms.ops.standard_normal((batch_size, nz))
