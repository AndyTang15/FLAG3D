...
class STGCN(nn.Cell):
    ...
    def __init__(self, in_channels, num_class, graph_args,
        ...
        A = ms.Tensor(self.graph.A, dtype=ms.float32)
        self._set_attr_for_tensor('A', A)
        ...
        if edge_importance_weighting:
            self.edge_importance = ms.ParameterTuple([
                ms.Parameter(ops.ones(self.A.shape, ms.int32))
    ...
    def construct(self, batch):
        ...
        x = batch["x"].permute(0, 2, 3, 1).unsqueeze(4)
        ...
        # global pooling
        x = ops.avg_pool2d(x, x.shape[2:])
        x = x.view(N, M, -1, 1, 1).mean(axis=1)
    ...
    def compute_accuracy(self, batch):
        confusion = ops.zeros((self.num_class, self.num_class), ms.int64)
        yhat = batch["yhat"].max(dim=1).indices
        ygt = batch["y"]
        for label, pred in zip(ygt, yhat):
            confusion[label][pred] += 1
        accuracy = ms.Tensor.trace(confusion) / ops.reduce_sum(confusion.astype(ms.float32))
        return accuracy
    ...
    def compute_loss(self, batch):
        ...
        losses = {"cross_entropy": cross_entropy.numpy().item(),
                  "mixed": mixed_loss.numpy().item(),
